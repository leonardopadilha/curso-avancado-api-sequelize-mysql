...
class PessoaController {
  ...
  //soft delete (restaura - exclusão suave)
  static async restauraPessoa(req, res) {
    ...
    try {
      ...
      return res.status(201).json(`id ${id} restaurado com sucesso!`)
  ...
  // Contar a quantidade de matrículas
  static async pegaMatriculaPorTurma(req, res) {
    ...
    try {
      const todasAsMatriculas = await database.Matriculas.findAndCountAll({
        ...
        limit : 20, //Utilizado em casos de paginação
  ...
  static async cancelaPessoa(req, res) {
    ...
    try {
      database.sequelize.transaction(async transacao => {
        await database.Pessoas.update({ ativo : false }, { where : {
          id : Number(estudanteId) }}, 
          { transaction : transacao} )
  
        await database.Matriculas.update({status : 'cancelado'}, { where : {
          estudante_id : Number(estudanteId) }},
          { transaction : transacao })
